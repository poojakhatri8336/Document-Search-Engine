...
collection = db.CNN_collectionProxy
collection.create_index('URL', unique=True)

searchlist = ['india', 'weather', 'world', 'business', 'entertainment', 'style', 'sports', 'food', 'design', 'opinion',
              'success', 'shooting', 'culture', 'media', 'china', 'Europe', 'Middle East', 'Pandemic', 'Australia',
              'US', 'congress', 'America', 'film', 'crime', 'football', 'Energy']
searchlist3 = [
    'mountain']  # luxury and tennis  5000 ,skiing , horse racing and parenting 1100,  dubai 3200, beauty 6800, 'United Kingdom' upto 156 page, drink basketball soccer california apartments transportation beach
...
for word in searchlist:
    ...
    for page in range(1, 201):  # specify how manytotal page from pagination you want to extract
        ...
        for i in range(0, 50):  # bcoz one page has 50 records
            try:
                mydict = {"URL": data[i]['url'], "Headline": data[i]['headline'], "Body": data[i]['body'],
                          "LastPublishDate": data[i]['lastPublishDate']}
                # print(mydict)
